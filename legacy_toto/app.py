# SKYTOTO AI HUB DUAL ANALYSIS REAL ENGINE - Python 3.14 Safe
# 목적: 외부 구매앱을 보면서, 같은 경기 기준으로 허브 분석 결과와 구글시트/과거자료 분석 결과를 동시에 비교하는 분석 보조앱.
# 자동구매 기능 없음. 구매 버튼 없음.
...
APP_TITLE = "SKYTOTO AI HUB 듀얼 분석 TACTICAL ENGINE"
APP_VERSION = "PY314_TACTICAL_ENGINE_V3_AUDIT_BUFFER_SAFE"
KST = timezone(timedelta(hours=9))
BASE_DIR = Path(__file__).resolve().parent
MEMORY_PATH = BASE_DIR / "ai_memory.json"
HUB_LOG_PATH = BASE_DIR / "hub_result_log.jsonl"
SHEET_LOG_PATH = BASE_DIR / "sheet_result_log.jsonl"
...
def cache_data_safe(**kwargs):
    """Streamlit cache wrapper. 테스트 환경/구버전 Streamlit에서는 원본 함수로 안전 통과."""
    cache_func = getattr(st, "cache_data", None)
    if callable(cache_func):
        try:
            return cache_func(**kwargs)
        except Exception:
            pass
...
def append_jsonl(path: Path, data: Dict[str, Any]) -> bool:
    try:
...
@cache_data_safe(show_spinner=False, ttl=3600)
...
@cache_data_safe(show_spinner=False, ttl=300)
...
def try_save_result(result: AnalysisResult) -> bool:
    memory = ensure_memory()
    key = "hub_results" if result.source == "허브" else "sheet_results"
    result_dict = asdict(result)
    result_dict["result_id"] = stable_result_id(result)
    memory.setdefault(key, [])
    existing_ids = {r.get("result_id") for r in memory[key] if isinstance(r, dict)}
    if result_dict["result_id"] not in existing_ids:
        memory[key].append(result_dict)
        memory[key] = memory[key][-500:]
        ok_memory = save_json(MEMORY_PATH, memory)
        ok_log = append_jsonl(HUB_LOG_PATH if result.source == "허브" else SHEET_LOG_PATH, result_dict)
        return bool(ok_memory and ok_log)
    # 같은 결과가 rerun으로 반복되면 파일을 다시 쓰지 않고 성공 처리
    return True


def render_result_card(result: AnalysisResult) -> None:
    st.markdown(f"### {result.source} 결과")
    c1, c2, c3 = st.columns(3)
    c1.metric("예상", result.predicted_label)
    c2.metric("예상 점수", result.predicted_score)
    c3.metric("신뢰도", f"{result.confidence}%")
    c4, c5, c6 = st.columns(3)
    c4.metric("홈승", f"{result.home_prob}%")
    c5.metric("무승부", f"{result.draw_prob}%")
    c6.metric("원정승", f"{result.away_prob}%")
    st.warning(f"위험도: {result.risk} / 위험점수 {result.risk_score}") if result.risk != "낮음" else st.success(f"위험도: {result.risk} / 위험점수 {result.risk_score}")
    with st.expander(f"{result.source} 근거 / 경우의수 보기", expanded=False):
        st.write("근거")
        for reason in result.reasons:
            st.write("- " + reason)
        st.write("위험 신호")
        for danger in result.danger_cases:
            st.write("- " + danger)
        st.write("상위 예상 점수")
        st.dataframe(pd.DataFrame(result.top_scorelines), use_container_width=True, hide_index=True)
        st.write("경우의수 / 유사경기")
        st.dataframe(pd.DataFrame(result.top_cases), use_container_width=True, hide_index=True)


def render_pc_monitor(hub: AnalysisResult, sheet: AnalysisResult, comp: Dict[str, Any]) -> None:
    st.markdown("## 🖥 PC 확인 모니터")
    monitor = pd.DataFrame([
        {
            "경기": f"{hub.home_team} vs {hub.away_team}",
            "허브예상": hub.predicted_label,
            "허브점수": hub.predicted_score,
            "허브신뢰": hub.confidence,
            "시트예상": sheet.predicted_label,
            "시트점수": sheet.predicted_score,
            "시트신뢰": sheet.confidence,
            "비교": "일치" if comp["same"] else "불일치",
            "최종": comp["final"],
        }
    ])
    st.dataframe(monitor, use_container_width=True, hide_index=True)
    st.caption("PC는 확인 모니터 전용입니다. 저장/수정/구매 버튼을 두지 않습니다.")


def parse_manual_history(text: str) -> Optional[pd.DataFrame]:
...
def render_experiment_tab(history: pd.DataFrame) -> None:
    st.subheader("🧪 실험 테스트 / 백테스트 / 배당 위험 시나리오")
    st.caption("실제 자료가 부족해도 가짜 대량자료와 위험 시나리오로 버퍼링·분석·위험탐지 오류를 확인합니다.")
    c1,c2,c3=st.columns(3)
    with c1: synth_rows=st.number_input("가짜 과거자료 생성 건수", min_value=100, max_value=250000, value=50000, step=1000, key="experiment_v1_synth_rows")
    with c2: scenario=st.selectbox("위험 시나리오", ["mixed","low_odds_trap","draw_trap","odds_manipulation","away_upset"], key="experiment_v1_scenario")
    with c3: backtest_n=st.number_input("백테스트 표본 수", min_value=10, max_value=3000, value=300, step=10, key="experiment_v1_backtest_n")
    if not st.button("실험 테스트 실행", key="experiment_v1_run_button"):
        st.info("버튼을 누르면 대량자료 생성 → 정규화 → 허브/구글시트형 분석 → 백테스트 → 위험 시나리오 검사를 실행합니다.")
        return
    started=datetime.now()
    with st.spinner("실험자료 생성/백테스트 중..."):
        synth=generate_synthetic_history(int(synth_rows), scenario=str(scenario))
        combined=normalize_history_df(pd.concat([history, synth], ignore_index=True))
        bt=run_backtest(combined, int(backtest_n))
        scen=normalize_history_df(scenario_test_pack())
        scenario_rows=[]
        for _,row in scen.iterrows():
            inp=make_input_from_history_row(row); hub=hub_analyze(inp, combined); sheet=sheet_analyze(inp, combined); comp=compare_results(hub, sheet)
            scenario_rows.append({"시나리오":row.get("scenario_tag",""),"경기":f"{inp.home_team} vs {inp.away_team}","실제":OUTCOME_LABELS.get(actual_outcome(row.get("home_score"), row.get("away_score"))),"허브":hub.predicted_label,"구글시트형":sheet.predicted_label,"최종판정":comp.get("final",""),"위험신호":" / ".join(sorted(set(hub.danger_cases+sheet.danger_cases)))[:240]})
    elapsed=(datetime.now()-started).total_seconds()
    st.success(f"실험 완료: 통합 과거자료 {len(combined):,}건 / 소요 {elapsed:.2f}초")
    if bt.get("ok"):
        m1,m2,m3,m4=st.columns(4)
        m1.metric("허브 적중률", f"{bt['hub_accuracy']}%"); m2.metric("구글시트형 적중률", f"{bt['sheet_accuracy']}%"); m3.metric("두 결과 일치 시", f"{bt['same_accuracy']}%", f"{bt['same_count']}건"); m4.metric("불일치 시 한쪽 적중", f"{bt['diff_any_accuracy']}%", f"{bt['diff_count']}건")
        st.caption(f"배당/함정 위험 패턴이 실패와 겹친 건수: {bt['risk_fail_hits']}건")
        st.dataframe(pd.DataFrame(bt["rows"]).head(500), use_container_width=True, hide_index=True)
    else:
        st.error(bt.get("message", "백테스트 실패"))
    st.markdown("### 고정 위험 시나리오 검사"); st.dataframe(pd.DataFrame(scenario_rows), use_container_width=True, hide_index=True)
    st.markdown("### 생성자료 샘플"); st.dataframe(synth.head(300), use_container_width=True, hide_index=True)


def safe_app() -> None:
    st.set_page_config(page_title=APP_TITLE, page_icon="⚽", layout="wide")
    st.title("⚽ SKYTOTO 듀얼 분석 REAL ENGINE")
    st.caption("허브는 실시간 입력+선수/감독/전술까지 분석, 구글시트/과거자료는 대량 과거자료+유사경기+전술변수까지 따로 분석합니다. 자동구매 없음.")

    with st.sidebar:
        st.header("과거자료 입력")
        st.caption("핵심은 과거자료를 많이 넣는 것입니다. CSV/구글시트 CSV URL/붙여넣기를 지원합니다.")
        uploaded = st.file_uploader("과거 경기 CSV/엑셀 업로드", type=["csv", "xlsx"], key="real_v1_history_upload")
        csv_url = st.text_input("구글시트 CSV 공개 URL", value="", key="real_v1_sheet_url")
        manual_text = st.text_area("CSV 직접 붙여넣기", value="", height=120, key="real_v1_manual_csv")
        use_sample = st.checkbox("샘플 과거자료 같이 사용", value=True, key="real_v1_use_sample")
        st.caption("Secrets: 현재 버전은 공개 CSV/업로드 방식이라 필수 secrets 없음")

    dfs: List[pd.DataFrame] = []
    uploaded_df = load_history_from_csv_upload(uploaded)
    if uploaded_df is not None:
        dfs.append(uploaded_df)
    url_df = load_history_from_url(csv_url)
    if url_df is not None:
        dfs.append(url_df)
    manual_df = parse_manual_history(manual_text)
    if manual_df is not None:
        dfs.append(manual_df)
    if use_sample or not dfs:
        dfs.append(sample_history())
    raw_history = pd.concat(dfs, ignore_index=True) if dfs else sample_history()
    history = normalize_history_df(raw_history)

    st.info(f"현재 분석에 사용 가능한 과거자료: {len(history):,}건")
    if len(raw_history) > len(history):
        st.warning(f"버퍼링 방지: 원본 {len(raw_history):,}건 중 최근 {len(history):,}건만 분석에 사용")

    tab_mobile, tab_pc, tab_review, tab_data, tab_experiment = st.tabs(["📱 모바일 비교", "🖥 PC 모니터", "✅ 경기후 판정", "📚 과거자료 확인", "🧪 실험 테스트"])

    with tab_mobile:
        st.subheader("같은 경기 기준: 허브 결과 + 구글시트 결과 동시 비교")
        c1, c2 = st.columns(2)
        with c1:
            league = st.text_input("리그", value="월드컵", key="real_v1_league")
            home_team = st.text_input("홈팀", value="독일", key="real_v1_home")
            away_team = st.text_input("원정팀", value="파라과이", key="real_v1_away")
            match_minute = st.number_input("현재 경기 시간(분)", min_value=0, max_value=130, value=0, step=1, key="real_v1_minute")
            home_live_score = st.number_input("홈 현재 점수", min_value=0, max_value=20, value=0, step=1, key="real_v1_home_score")
            away_live_score = st.number_input("원정 현재 점수", min_value=0, max_value=20, value=0, step=1, key="real_v1_away_score")
        with c2:
            odds_home = st.number_input("홈승 배당", min_value=0.0, max_value=1000.0, value=1.80, step=0.01, key="real_v1_odds_home")
            odds_draw = st.number_input("무승부 배당", min_value=0.0, max_value=1000.0, value=3.40, step=0.01, key="real_v1_odds_draw")
            odds_away = st.number_input("원정승 배당", min_value=0.0, max_value=1000.0, value=4.20, step=0.01, key="real_v1_odds_away")
            home_form_score = st.slider("홈 최근 폼", 0, 100, 58, key="real_v1_home_form")
            away_form_score = st.slider("원정 최근 폼", 0, 100, 48, key="real_v1_away_form")
            home_attack = st.slider("홈 공격력", 0, 100, 60, key="real_v1_home_attack")
            away_attack = st.slider("원정 공격력", 0, 100, 48, key="real_v1_away_attack")
            home_defense_risk = st.slider("홈 수비위험", 0, 100, 40, key="real_v1_home_def_risk")
            away_defense_risk = st.slider("원정 수비위험", 0, 100, 55, key="real_v1_away_def_risk")
            market_note = st.text_input("배당/현장 메모", value="", key="real_v2_market_note")

        st.markdown("### 🧩 선수/감독/전술 변수")
        t1, t2, t3 = st.columns(3)
        with t1:
            home_main_injuries = st.number_input("홈 주전 부상", min_value=0, max_value=20, value=0, step=1, key="real_v2_home_main_injuries")
            away_main_injuries = st.number_input("원정 주전 부상", min_value=0, max_value=20, value=0, step=1, key="real_v2_away_main_injuries")
            home_suspended = st.number_input("홈 징계/결장", min_value=0, max_value=20, value=0, step=1, key="real_v2_home_suspended")
            away_suspended = st.number_input("원정 징계/결장", min_value=0, max_value=20, value=0, step=1, key="real_v2_away_suspended")
        with t2:
            home_bench_depth = st.slider("홈 대기선수/벤치 뎁스", 0, 100, 60, key="real_v2_home_bench")
            away_bench_depth = st.slider("원정 대기선수/벤치 뎁스", 0, 100, 55, key="real_v2_away_bench")
            home_tactic_fit = st.slider("홈 전술 적합도", 0, 100, 60, key="real_v2_home_tactic")
            away_tactic_fit = st.slider("원정 전술 적합도", 0, 100, 55, key="real_v2_away_tactic")
        with t3:
            home_coach_months = st.number_input("홈 감독 취임 개월", min_value=0, max_value=240, value=12, step=1, key="real_v2_home_coach_months")
            away_coach_months = st.number_input("원정 감독 취임 개월", min_value=0, max_value=240, value=12, step=1, key="real_v2_away_coach_months")
            home_rotation_risk = st.slider("홈 로테이션 위험", 0, 100, 35, key="real_v2_home_rotation")
            away_rotation_risk = st.slider("원정 로테이션 위험", 0, 100, 40, key="real_v2_away_rotation")
        n1, n2, n3 = st.columns(3)
        with n1:
            home_motivation = st.slider("홈 동기부여", 0, 100, 60, key="real_v2_home_motivation")
            away_motivation = st.slider("원정 동기부여", 0, 100, 55, key="real_v2_away_motivation")
        with n2:
            coach_note = st.text_area("감독 성향/취임 메모", value="", height=80, key="real_v2_coach_note")
        with n3:
            tactic_note = st.text_area("전술/전략 메모", value="", height=80, key="real_v2_tactic_note")
        lineup_note = st.text_area("선발/대기선수 명단 메모", value="", height=80, key="real_v2_lineup_note")

        inp = MatchInput(
            league=league,
            home_team=clean_team_name(home_team),
            away_team=clean_team_name(away_team),
            match_minute=safe_int(match_minute),
            home_live_score=safe_int(home_live_score),
            away_live_score=safe_int(away_live_score),
            odds_home=safe_float(odds_home),
            odds_draw=safe_float(odds_draw),
            odds_away=safe_float(odds_away),
            home_form_score=safe_float(home_form_score),
            away_form_score=safe_float(away_form_score),
            home_attack=safe_float(home_attack),
            away_attack=safe_float(away_attack),
            home_defense_risk=safe_float(home_defense_risk),
            away_defense_risk=safe_float(away_defense_risk),
            home_main_injuries=safe_int(home_main_injuries),
            away_main_injuries=safe_int(away_main_injuries),
            home_suspended=safe_int(home_suspended),
            away_suspended=safe_int(away_suspended),
            home_bench_depth=safe_float(home_bench_depth),
            away_bench_depth=safe_float(away_bench_depth),
            home_tactic_fit=safe_float(home_tactic_fit),
            away_tactic_fit=safe_float(away_tactic_fit),
            home_coach_months=safe_int(home_coach_months),
            away_coach_months=safe_int(away_coach_months),
            home_rotation_risk=safe_float(home_rotation_risk),
            away_rotation_risk=safe_float(away_rotation_risk),
            home_motivation=safe_float(home_motivation),
            away_motivation=safe_float(away_motivation),
            coach_note=coach_note,
            tactic_note=tactic_note,
            lineup_note=lineup_note,
            market_note=market_note,
        )
        hub_result = hub_analyze(inp, history)
        sheet_result = sheet_analyze(inp, history)
        comparison = compare_results(hub_result, sheet_result)

        st.markdown(f"## {comparison['icon']} 최종 비교: {comparison['final']}")
        st.write(" / ".join(comparison["notes"]))
        r1, r2 = st.columns(2)
        with r1:
            render_result_card(hub_result)
        with r2:
            render_result_card(sheet_result)

        s1 = try_save_result(hub_result)
        s2 = try_save_result(sheet_result)
        st.caption(f"자동 기록 상태: 허브 {'완료' if s1 else '실패'} / 구글시트형 {'완료' if s2 else '실패'} / {now_kst_str()}")

    with tab_pc:
        render_pc_monitor(hub_result, sheet_result, comparison)

    with tab_review:
        st.subheader("경기 후 성공/실패 판정")
        actual_home = st.number_input("실제 홈 점수", min_value=0, max_value=30, value=0, step=1, key="real_v1_actual_home")
        actual_away = st.number_input("실제 원정 점수", min_value=0, max_value=30, value=0, step=1, key="real_v1_actual_away")
        hub_review = review_prediction(hub_result, safe_int(actual_home), safe_int(actual_away))
        sheet_review = review_prediction(sheet_result, safe_int(actual_home), safe_int(actual_away))
        st.write("허브 판정")
        st.json(hub_review)
        st.write("구글시트형 판정")
        st.json(sheet_review)
        memory = ensure_memory()
        memory.setdefault("reviews", [])
        memory["reviews"].extend([hub_review, sheet_review])
        memory["reviews"] = memory["reviews"][-500:]
        save_json(MEMORY_PATH, memory)

    with tab_data:
        st.subheader("과거자료 확인")
        st.dataframe(history.tail(300), use_container_width=True, hide_index=True)
        st.caption("기본 컬럼: date, league, home_team, away_team, home_score, away_score, odds_home, odds_draw, odds_away")
        st.caption("전술 확장 컬럼: home_main_injuries, away_main_injuries, home_suspended, away_suspended, home_bench_depth, away_bench_depth, home_tactic_fit, away_tactic_fit, home_coach_months, away_coach_months, home_rotation_risk, away_rotation_risk, home_motivation, away_motivation")


    with tab_experiment:
        render_experiment_tab(history)
...
def main() -> None:
    try:
        ...
        st.set_page_config(page_title="SKYTOTO 안전모드", page_icon="⚠️", layout="wide")
        st.error("앱 실행 중 오류가 발생했지만 안전모드로 중단을 막았습니다.")
        st.code(str(exc))
